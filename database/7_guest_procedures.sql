...
BEGIN
  /* Choose one trainer per stable deterministically (alphabetically first) */
  SELECT
    h.horseName,
    h.age,
...
BEGIN
  /* Sum prizes for winning results per chosen stable trainer */
  WITH pick AS (
    SELECT stableId, MIN(CONCAT(lname, ',', fname)) AS pick
    FROM Trainer
    GROUP BY stableId
